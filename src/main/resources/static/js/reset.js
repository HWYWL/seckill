$(function(){
	...
	$("#pc_tel").blur(function () {
        ...
        if(checkInput(pc_tel_con)){
        	// if (dtype == 'email') {
        		$(".pc-code").find('img').attr('src','/user/verifyCode?'+Math.random()).click(function(event) {
	            	$(this).attr('src','/user/verifyCode?'+Math.random());
	            });
	            // $('.pc-code').find('a').click(function(event) {
	            // 	$('.pc-code').find('img').attr('src','/user/verifyCode?'+Math.random());
	            // });
        	// }
        }
    });
 		...
 		if (status) {
	 		if (checkcode(dtype, code)) {
	 			$.ajax({
		            ...
		            dataType: 'json',
		            async: true,
		            data: {inp:inp,code:code,type:dtype},
		            success:function(data){
		                if (data.code == '0') {
		                	$('.pc_tel-err').removeClass('hide');
    ...
    $(".finish").click(function () {
	    ...
	    if (oReset_sure != oReset) {
   		...
   		$.ajax({
            ...
            dataType: 'json',
            async: true,
            data: {newpass:oReset,repass:oReset_sure},
            success:function(data){
                if (data.code == '0') {
                	globalTip(data);
                } else {
    ...
    $(".form-data").delegate(".send","click",function () {
    	...
    	if (dtype == 'phone' && status) {
    		$.ajax({
	            ...
	            dataType: 'json',
	            async: true,
	            data: {phone:inp,type:"reset",note:note,code:code},
	            success:function(data){
	                if (data.code == '0') {
